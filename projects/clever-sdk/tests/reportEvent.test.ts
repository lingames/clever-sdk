...
import { TiktokGameEvent, TiktokEventParams } from "../src/models";
...
describe("TiktokSdk.reportEvent 类型推断（编译期验证）", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        const tt = getTTMinis();
    ...
    it("缺失 section_type 编译期拒绝", () => {
        const sdk = createMockTiktokSdk();
        // @ts-expect-error: Property 'section_type' is missing
        sdk.reportEvent("level_1", {
            event_type: TiktokGameEvent.COMPLETE_SECTION,
        });
    });
    ...
    it("向后兼容：不传 event_type 走 Record<string, any>", () => {
        const sdk = createMockTiktokSdk();
        const legacyCall = () =>
            sdk.reportEvent("legacy_event", {
                foo: "bar",
                count: 42,
            });
    ...
    it("向后兼容：event_type 为未知字符串走 Record<string, any>", () => {
        const sdk = createMockTiktokSdk();
        const unknownEventCall = () =>
            sdk.reportEvent("some_event", {
